...
import { Gift, Timer, Users, Loader2 } from "lucide-react";
import {
  getActiveEventSlot,
...
export default function EventParticipationPopup() {
  ...
  const handleJoin = () => {
    if (!session || !session.user?.id || session.user.name === "Tamu") {
      setIsOpen(false);
          ...
          <div className="relative flex flex-col items-center text-center">
            ...
            <div className="mt-8 w-full flex flex-col gap-3">
              ...
              <button
                ...
                className="text-muted-foreground text-sm transition-colors cursor-pointer"
              >
                Gunakan mode tamu
              </button>
            </div>
