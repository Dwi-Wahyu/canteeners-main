...
export function CartDrawer({ cart_id }: { cart_id?: string; }) {
    ...
    const totalItemCount = cart?.shop_carts.reduce((acc, shopCart) => {
        return acc + (shopCart._count?.items ?? 0);
    }, 0) ?? 0;

    const totalPrice = cart?.shop_carts.reduce((acc, shopCart) => {
        ...
        const shopTotal = shopCart.items.reduce((sAcc, item) => sAcc + item.subtotal, 0);
                ...
                <div className="mx-auto w-full flex flex-col h-full">
                    <DrawerHeader>
                        ...
                        <DrawerDescription className="text-start">
                            Review item yang akan dibeli dari berbagai kedai.
                        </DrawerDescription>
                    </DrawerHeader>
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center flex-1 p-8 text-center space-y-4">
                            ...
                            <DrawerClose asChild>
                                <Link href="/kantin" className="w-full">
                        ...
                        <>
                            <ScrollArea className="flex-1 px-4 overflow-y-auto">
                                <div className="space-y-6 pb-6">
                                    {cart?.shop_carts.map((shopCart) => (
                                        shopCart.items.length > 0 && (
                                            <div key={shopCart.id} className="space-y-3">
                                                <div className="flex items-center gap-2 pb-2 border-b">
                                                    <Store className="w-4 h-4 text-primary" />
                                                    <span className="font-semibold text-sm">{shopCart.shop.name}</span>
                                                </div>
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                ...
                                                                {item.selected_options.length > 0 && (
                                                                    <p className="text-xs text-muted-foreground truncate">
                                                                        {item.selected_options.map(opt => opt.value).join(', ')}
                                                                    </p>
                                                                )}
                                                                <div className="flex items-center justify-between mt-1">
                                                                    <p className="text-xs text-muted-foreground">
                                                                        {item.quantity} x Rp {((item.subtotal / item.quantity) || 0).toLocaleString('id-ID')}
                                                                    </p>
                            ...
                            <div className="p-4 border-t bg-background mt-auto">
                                ...
                                <DrawerFooter className="p-0">
                                    <DrawerClose asChild>
                                        <Link href={cart.shop_carts.length === 1 ? "/keranjang/" + cart.shop_carts[0].id : "/keranjang"} className="w-full">
